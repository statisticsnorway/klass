...
/**
 * Codelists fetched from stabas web service does not include shortname for classification items.
 * And is also missing include/include_also/excludes. This class patches classificationItems with
 * missing shortname, include, include_also and excludes.
 */
class ShortnamePatcher {
    ...
    public void patch(String versionId, ClassificationVersionWrapper version) {
        for (PatchRow patch : findPatchesForVersion(versionId)) {
            ConcreteClassificationItem item =
                    (ConcreteClassificationItem) version.findItemWithStabasId(patch.getItemId());
            patchShortTitle(item, toTranslatable(patch.getShortTitle()));
            patchNotes(
                    item,
                    toTranslatable(patch.getIncludes()),
                    toTranslatable(patch.getIncludesAlso()),
    ...
    private List<PatchRow> readPatchFile(File patchfile) {
        ...
        ObjectReader reader = mapper.readerFor(PatchRow.class).with(schema).withoutRootName();
        try {
    ...
    private List<PatchRow> findPatchesForVersion(String stabasVersionId) {
        return patches.stream()
                .filter(patch -> patch.getVersionId().equals(stabasVersionId))
                .collect(toList());
    }

    private void patchNotes(
            ConcreteClassificationItem item,
            Translatable includes,
            Translatable includesAlso,
