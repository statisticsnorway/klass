...
public class CorrespondenceTable extends BaseEntity
   ...
   *
   * @param map
   * @return true if already contains a correspondenceMap with identical source and target mapping,
   *     false otherwise
   */
  ...
  public LocalDate getOccured() {
    return TimeUtil.max(
        Lists.newArrayList(
            getSource().getDateRange().getFrom(), getTarget().getDateRange().getFrom()));
  }
  ...
  public DateRange getDateRange() {
    ...
    LocalDate from =
        TimeUtil.max(
            Lists.newArrayList(source.getDateRange().getFrom(), target.getDateRange().getFrom()));
    LocalDate to =
        TimeUtil.min(
            Lists.newArrayList(source.getDateRange().getTo(), target.getDateRange().getTo()));
