...
public interface SubscriberRepository extends JpaRepository<Subscriber, Long> {
    ...
    @Query(value = "select s  from Subscriber s join s.subscriptions as su where su.verification = 'VALID'")
    List<Subscriber> findAllSubscribers();

    @Query(value = "select count(*) from Subscriber s join s.subscriptions as su where su.verification = 'VALID'")
    int findNumberOfSubscribers();

    @Query(value = "select count(*) from Subscriber s join s.subscriptions as su where s.email LIKE '%ssb.no%' and su.verification = 'VALID'")
    int findNumberOfInternalSubscribers();

    @Query(value = "select count(*) from Subscriber s join s.subscriptions as su where s.email NOT LIKE '%ssb.no%' and su.verification = 'VALID'")
