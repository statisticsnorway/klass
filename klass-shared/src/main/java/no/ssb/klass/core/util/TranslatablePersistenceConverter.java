...
public class TranslatablePersistenceConverter implements AttributeConverter<Translatable, String> {
  ...
  public TranslatablePersistenceConverter(String no, String nn, String en) {
    this.noPattern =
        Pattern.compile("<" + no + ">(.*)</" + no + ">", Pattern.MULTILINE | Pattern.DOTALL);
    this.nnPattern =
        Pattern.compile("<" + nn + ">(.*)</" + nn + ">", Pattern.MULTILINE | Pattern.DOTALL);
    this.enPattern =
        Pattern.compile("<" + en + ">(.*)</" + en + ">", Pattern.MULTILINE | Pattern.DOTALL);
  }
