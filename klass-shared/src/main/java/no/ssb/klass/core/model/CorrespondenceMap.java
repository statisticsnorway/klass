...
@Table(
    uniqueConstraints =
        @UniqueConstraint(columnNames = {"source_id", "target_id", "correspondence_table_id"}))
public class CorrespondenceMap extends BaseEntity implements Comparable<CorrespondenceMap> {
  ...
  public int compareTo(CorrespondenceMap other) {
    int compare =
        nullSafeStringComparator.compare(getCodeFromItem(source), getCodeFromItem(other.source));
    if (compare == 0) {
      compare =
          nullSafeStringComparator.compare(getCodeFromItem(target), getCodeFromItem(other.target));
    }
    return compare;
  }
