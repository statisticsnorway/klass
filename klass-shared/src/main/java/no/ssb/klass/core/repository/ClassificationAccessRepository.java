...
@Repository
public interface ClassificationAccessRepository
    extends JpaRepository<ClassificationAccessCounter, Long> {
  @Query(
      value =
          "select count(*) from ClassificationAccessCounter c "
              + "where c.timeStamp between :fromDate and :toDate")
  int getAccessSum(@Param("fromDate") Date fromDate, @Param("toDate") Date toDate);

  @Query(
      value =
          "select new no.ssb.klass.core.service.dto.AccessCounterDto("
