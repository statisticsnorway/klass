...
public class ClassificationServiceImpl implements ClassificationService {
  ...
  public ClassificationSeries getClassificationSeries(Long id) {
    ClassificationSeries classificationSeries =
        classificationRepository
            ...
            .orElseThrow(
                () ->
                    new KlassResourceNotFoundException("Classification not found with id = " + id));
  ...
  private void deleteAnyDeletedReferencesToClassificationItems(ClassificationVersion version) {
    ...
    softDeletedItems.forEach(
        item -> item.getLevel().getStatisticalClassification().deleteClassificationItem(item));
  ...
  public List<CorrespondenceDto> findCorrespondences(
    ...
    if (correspondences.isEmpty())
      throw new KlassResourceNotFoundException(
          createCorrespondenceNotFoundErrorMessage(sourceClassification, targetClassification));
  ...
  public void deleteNotIndexClassification(User user, ClassificationSeries classification)
    ...
    user =
        userRepository
            .findById(user.getId())
            .orElseThrow(() -> new RuntimeException("User not found")); // get attached object
  ...
  public Set<String> findReferencesOfClassificationItem(ClassificationItem classificationItem) {
    ...
    Set<String> result =
        items.stream()
            .map(item -> item.getLevel().getStatisticalClassification().getNameInPrimaryLanguage())
            .collect(toSet());
  ...
  public Set<String> findReferencesOfClassificationItems(
    ...
    Set<CorrespondenceTable> referencingCorrespondenceTables =
        correspondenceMapRepository.findAllMapsUsingItems(statisticalClassification).stream()
            ...
            .map(
                table ->
                    table.getNameInPrimaryLanguage()
                        + " ID="
                        + table.getId()) // Also returns ID for error printing
            .collect(toSet()));
