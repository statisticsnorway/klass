...
public interface CorrespondenceMapRepository extends CrudRepository<CorrespondenceMap, Long> {
     ...
     * @param deleted
     *            flag that decides if deleted or not deleted CorrespondenceMaps are returned
     *
     * @return list of CorrespondenceMaps whose source or target references provided classificationItem
     */
    @Query("FROM no.ssb.klass.core.model.CorrespondenceMap map "
            + "where (map.source = :item OR map.target = :item)and map.correspondenceTable.deleted = :deleted")
    Set<CorrespondenceMap> findBySourceOrTarget(@Param("item") ClassificationItem item,
            @Param("deleted") boolean deleted);

    /**
     * Finds all CorrespondenceMaps whose source or target references a classificationItem in provided statistical
     * classification
     *
     * @param classification
     *            StatisticalClassification with items to match
     *
     * @return list of CorrespondenceMaps whose source or target references one or more classificationItem in provided
     *         statistical classification
     */
    @Query("SELECT map FROM no.ssb.klass.core.model.CorrespondenceMap map "
    ...
    Set<CorrespondenceMap> findAllMapsUsingItems(@Param("version") StatisticalClassification classification);

    /**
     * Finds all CorrespondenceMaps whose source or target references a deleted classificationItem from provided
     * statistical classification
     *
     * @param classification
     *            StatisticalClassification with items to match
     * @param deletedClassificationItems
     *            Since deletedItems is a transient collection you will have to provide it manually here
     *
     * @return list of CorrespondenceMaps whose source or target references one or more classificationItem in provided
     *         statistical classification and is found in list of deleted items.
     */
    @Query("SELECT map FROM no.ssb.klass.core.model.CorrespondenceMap map "
            + " INNER JOIN map.source source "
            + " INNER JOIN map.target target "
