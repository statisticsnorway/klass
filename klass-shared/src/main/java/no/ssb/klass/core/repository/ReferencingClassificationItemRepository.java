...
public interface ReferencingClassificationItemRepository
   ...
   * @param deleted flag to indicate if we want to look for soft deleted items (only deleted / only
   *     not deleted)
  ...
  List<ReferencingClassificationItem> findByReference(
      @Param("item") ClassificationItem classificationItem, @Param("deleted") boolean deleted);
