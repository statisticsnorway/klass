...
public interface ReferencingClassificationItemRepository extends Repository<ReferencingClassificationItem, Long> {
     ...
     * @param deleted
     *            flag to indicate if we want to look for soft deleted items (only deleted / only not deleted)
     *
     * @return list of ReferencingClassificationItem referencing input classificationItem
     */
    @Query("from no.ssb.klass.core.model.ReferencingClassificationItem item "
            + "where item.reference = :item and item.level.statisticalClassification.deleted = :deleted")
    List<ReferencingClassificationItem> findByReference(@Param("item") ClassificationItem classificationItem,
     ...
     *            list of classificationItems to match
     * @param deleted
     *            flag to indicate if we want to look for soft deleted items (only deleted / only not deleted)
     *
    ...
    List<ReferencingClassificationItem> findByReferenceInList(
            @Param("items") List<ClassificationItem> classificationItems, @Param("deleted") boolean deleted);

    /**
     * Finds all ReferencingClassificationItems that references classificationItems in given StatisticalClassification
     *
     * @param statisticalClassification
     *            StatisticalClassification with items to find references for
     * @param deleted
     *            flag to indicate if we want to look for soft deleted Referencing items (only deleted / only not
     *            deleted)
     *
     * @return list of ReferencingClassificationItem referencing classificationItems in StatisticalClassification
     */
