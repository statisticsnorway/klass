...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public ContentUseStatisticDto generateContentUseStat(
            String section, ClassificationType classificationType) {
        int numberOfClassifications =
                classificationRepository.finNumberOfClassifications(classificationType, section);
        int numberOfPublishedClassifications =
                classificationRepository.findNumberOfPublishedClassifications(
                        classificationType, section);

        int numberOfPublishedVersionsAnyLanguages =
                classificationRepository.findNumberOfPublishedVersionsAnyLanguages(
                        classificationType, section);

        int numberOfPublishedVersionsAllLanguages =
                classificationRepository.findNumberOfPublishedVersionsAllLanguages(
                        classificationType, section);

        return new ContentUseStatisticDto(
                numberOfClassifications,
                numberOfPublishedClassifications,
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ClassificationReportDto> getPublishedClassificationReport(
            String section, ClassificationType classificationType, ReportMode operation) {

        switch (operation) {
            case TOTAL:
                return classificationRepository.getClassificationReport(
                        classificationType, section);
            case PUBLISHED:
                return classificationRepository.getPublishedClassificationReport(
                        classificationType, section);
    ...
    @Override
    @Transactional(readOnly = true)
    public List<ClassificationVersionReportDto> getPublishedVersionReport(
            String section, ClassificationType classificationType, ReportMode operation) {

        switch (operation) {
            case ANY_LANGUAGE:
                return classificationRepository.getPublishedVersionsAnyLanguages(
                        classificationType, section);

            case ALL_LANGUAGE:
                return classificationRepository.getPublishedVersionsAllLanguages(
                        classificationType, section);
            default:
                throw new IllegalStateException("Gitt operasjon ikke implementert: " + operation);
        }
    ...
    public UsageStatisticsDto getUsageStatistics(LocalDate fromSearchDate, LocalDate toSearchDate) {
        ...
        int usageSumC =
                classificationAccessRepository.getAccessSum(Date.from(fromDate), Date.from(toDate));
        int numberOfMisses =
                searchWordsRepository.getNumberOfMiss(Date.from(fromDate), Date.from(toDate));
        int numberOfSearchWords =
                searchWordsRepository.getNumberOfSearchWords(
                        Date.from(fromDate), Date.from(toDate));
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<StatisticalEntity> getUsageStatistics(
        ...
        switch (operation) {
            case TOTAL_CLASSIFIC:
                return classificationAccessRepository.getClassificationsCount(
                        Date.from(fromDate), Date.from(toDate), pageable);
            case NUMBEROF_SEARCH_RETURNED_NULL:
                return searchWordsRepository.getSearchWords(
                        MISSED, Date.from(fromDate), Date.from(toDate), pageable);
    ...
    public SubscriberStatisticsDto getSubscriberStatistics() {
        ...
        return new SubscriberStatisticsDto(
                numberOfSubscribers, numberOfInternalSubscribers, numberOfExternalSubscribers);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<StatisticalEntity> getSubscriberClassificationReport(
            SubscriberMode operation, Pageable p) {
        switch (operation) {
            case TOTAL:
