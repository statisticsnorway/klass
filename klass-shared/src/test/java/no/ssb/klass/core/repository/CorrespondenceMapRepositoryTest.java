...
public class CorrespondenceMapRepositoryTest {
  ...
  public void findAllMapsUsingDeletedItems() {
    ...
    sourceMapsUsingDeletedItems.forEach(
        map ->
            assertTrue(deletedSource.getAllClassificationItems().contains(map.getSource().get())));
    targetMapsUsingDeletedItems.forEach(
        map ->
            assertTrue(deletedTarget.getAllClassificationItems().contains(map.getTarget().get())));
  }
  ...
  public void findAllMapsUsingItems() {
    ...
    // when
    Set<CorrespondenceMap> itemsUsingNormalSource =
        correspondenceMapRepository.findAllMapsUsingItems(normalSource);
    Set<CorrespondenceMap> itemsUsingNormalTarget =
        correspondenceMapRepository.findAllMapsUsingItems(normalTarget);

    // then
    assertThat(itemsUsingNormalSource.size()).isEqualTo(12);
    assertThat(itemsUsingNormalTarget.size()).isEqualTo(12);
    itemsUsingNormalSource.forEach(
        map ->
            assertTrue(normalSource.getAllClassificationItems().contains(map.getSource().get())));
    itemsUsingNormalSource.forEach(
        map ->
            assertTrue(normalSource.getAllClassificationItems().contains(map.getSource().get())));
  }
  ...
  public void findBySourceOrTarget() {
    ...
    // when
    Set<CorrespondenceMap> deletedFromNormal =
        correspondenceMapRepository.findBySourceOrTarget(classificationItem, true);
    Set<CorrespondenceMap> normalFromNormal =
        correspondenceMapRepository.findBySourceOrTarget(classificationItem, false);

    Set<CorrespondenceMap> deletedFromDeleted =
        correspondenceMapRepository.findBySourceOrTarget(deletedClassificationItem, true);
    Set<CorrespondenceMap> normalFromDeleted =
        correspondenceMapRepository.findBySourceOrTarget(deletedClassificationItem, false);
  ...
  private CorrespondenceTable createAndSaveCorrespondenceTable(
