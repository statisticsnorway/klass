...
public class ClassificationServiceImplTest {
  ...
  public void findByIdClassificationSeriesWithNameFiltersDeleted() {
    ...
    // then
    assertEquals(
        false, subject.findOneClassificationSeriesWithName("name", Language.NB).isPresent());
    assertEquals(
        false, subject.findOneClassificationSeriesWithName("name", Language.NN).isPresent());
    assertEquals(
        false, subject.findOneClassificationSeriesWithName("name", Language.EN).isPresent());
  }
  ...
  public void copyClassificationVersion() throws Exception {
    ...
    assertEquals(
        derived.getString(Language.NB), classificationVersionCopy.getDerivedFrom(Language.NB));
    assertEquals(
        legalbase.getString(Language.NB), classificationVersionCopy.getLegalBase(Language.NB));
    assertEquals(
  ...
  public void findCorrespondenceTablesBetween() {
    ...
    CorrespondenceTable correspondenceTable =
        TestUtil.createCorrespondenceTable("description", sourceVersion, 0, targetVersion, 0);
  ...
  public void findCorrespondenceTablesBetweenWithSpecifiedLevels() {
    ...
    CorrespondenceTable correspondenceTable =
        TestUtil.createCorrespondenceTable("description", sourceVersion, 1, targetVersion, 1);
    when(correspondenceTableRepositoryMock.findBySourceInAndTargetIn(anyList(), anyList()))
        .thenReturn(Lists.newArrayList(correspondenceTable));
