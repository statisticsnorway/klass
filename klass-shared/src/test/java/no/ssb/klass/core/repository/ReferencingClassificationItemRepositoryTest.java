...
public class ReferencingClassificationItemRepositoryTest {
    ...
    @Autowired private ClassificationVariantRepository classificationVariantRepository;
    ...
    public void findByReference() {
        ...
        ClassificationItem deletedClassificationItem =
                deletedDeleted.getAllClassificationItems().get(0);

        List<ReferencingClassificationItem> deletedItemReferences =
                referencingItemRepository.findByReference(classificationItem, true);
        List<ReferencingClassificationItem> normalItemReferences =
                referencingItemRepository.findByReference(classificationItem, false);

        assertThat(deletedItemReferences.size()).isEqualTo(0);
        assertThat(normalItemReferences.size()).isEqualTo(1);

        List<ReferencingClassificationItem> deletedReferencesForDeletedVariant =
                referencingItemRepository.findByReference(deletedClassificationItem, true);
        List<ReferencingClassificationItem> normalReferencesForDeletedVariant =
                referencingItemRepository.findByReference(deletedClassificationItem, false);
    ...
    public void findByReferenceInList() {
        ...
        List<ReferencingClassificationItem> deletedItemReferences =
                referencingItemRepository.findByReferenceInList(
        ...
        List<ReferencingClassificationItem> deletedReferencesForDeletedVariant =
                referencingItemRepository.findByReferenceInList(
                        deletedDeleted.getAllClassificationItems(), true);
        List<ReferencingClassificationItem> normalReferencesForDeletedVariant =
                referencingItemRepository.findByReferenceInList(
                        deletedDeleted.getAllClassificationItems(), false);
    ...
    public void findItemReferences() {
        ...
        List<ReferencingClassificationItem> deletedReferences =
                referencingItemRepository.findItemReferences(versionForNormalVariant, true);
        List<ReferencingClassificationItem> normalReferences =
                referencingItemRepository.findItemReferences(versionForNormalVariant, false);

        assertThat(deletedReferences.size()).isEqualTo(0);
        assertThat(normalReferences.size()).isEqualTo(12);

        List<ReferencingClassificationItem> deletedReferencesForDeletedVariant =
                referencingItemRepository.findItemReferences(versionForDeletedVariant, true);
        List<ReferencingClassificationItem> normalReferencesForDeletedVariant =
                referencingItemRepository.findItemReferences(versionForDeletedVariant, false);
    ...
    private ClassificationVariant createAndSaveVariant(
            String name, int itemCount, boolean deleted) {
        ClassificationVersion source = createAndSaveClassificationWithVersion(name, itemCount);
        ClassificationVariant variant =
                TestUtil.createClassificationVariant(name + "_variant", user);
    ...
    private ClassificationVersion createAndSaveClassificationWithVersion(
            String classificationName, int itemCount) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            ConcreteClassificationItem item =
                    new ConcreteClassificationItem(classificationName + i, name, name);
