...
public class CorrespondenceItemList {
    ...
    public CorrespondenceItemList(
            String csvSeparator, boolean displayWithValidRange, boolean includeFuture) {
    ...
    public CorrespondenceItemList limit(DateRange dateRange) {
        return newList(
                correspondenceItems.stream()
                        .map(
                                i ->
                                        new RangedCorrespondenceItem(
                                                i,
                                                i.getDateRange(includeFuture).subRange(dateRange)))
                        .collect(toList()));
    }

    public CorrespondenceItemList group() {
        Map<RangedCorrespondenceItem, List<RangedCorrespondenceItem>> grouped =
                correspondenceItems.stream().collect(groupingBy(i -> i));
    ...
    public List<RangedCorrespondenceItem> merge() {
        ...
        return ranges.stream()
                .map(i -> new RangedCorrespondenceItem(correspondenceItems.get(0), i))
                .collect(toList());
    }
