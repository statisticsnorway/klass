...
public class CodeList {
    ...
    private List<RangedCodeItem> combineCodeItems(
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (nextId < size) {
                ...
                // due to sorting only the last item should in theory be null if no requestRange end
                // date is
                // provided
    ...
    public CodeList filterOnLevel(String level) {
        ...
        return newCodeList(
                codeItems.stream()
                        .filter(codeItem -> level.equals(codeItem.getLevel()))
                        .collect(toList()));
    }
    ...
    public CodeList filterOnCodes(String selectCodes) {
        ...
        String pattern =
                selectCodes
                        .replaceAll(
                                RANGE_REGEX, "") // remove ranges (stored in map above(findRanges))
