...
@JsonPropertyOrder(
    value = {"code", "parentCode", "level", "name", "shortName", "notes", "validFrom", "validTo"})
public class ClassificationItemResource implements Comparable<ClassificationItemResource> {
  ...
  public ClassificationItemResource(
    ...
    this.parentCode =
        classificationItem.getParent() == null ? "" : classificationItem.getParent().getCode();
  ...
  public static List<ClassificationItemResource> convert(
      ClassificationVariant variant, Language language) {
    return variant.getLevels().stream()
        .flatMap(level -> convert(level, language).stream())
        .sorted()
        .collect(toList());
  }

  public static List<ClassificationItemResource> convert(
      ClassificationVersion version, Language language) {
    ...
    return version.getLevels().stream()
        .flatMap(
            level ->
                convert(level, language, includeShortName, includeNotes, includeValidity).stream())
        .sorted()
        .collect(toList());
  }
  ...
  private static List<ClassificationItemResource> convert(
      Level level,
      Language language,
      boolean includeShortName,
      boolean includeNotes,
    ...
    return level.getClassificationItems().stream()
        .map(
            item ->
                new ClassificationItemResource(
                    item, level, language, includeShortName, includeNotes, includeValidity))
        .collect(toList());
  }
}
