...
public class ClassificationVersionResource extends ClassificationVersionSummaryResource {
  ...
  public ClassificationVersionResource(
    ...
    List<CorrespondenceTable> corrTableVersionIsSource =
        version.getPublicCorrespondenceTables().stream()
            .sorted(
    ...
    if (!includeFuture) {
      publicCorrespondenceTables =
          publicCorrespondenceTables.stream()
              .filter(this::filterFuture)
              .collect(Collectors.toList());
    }

    this.correspondenceTables =
        CorrespondenceTableSummaryResource.convert(publicCorrespondenceTables, language);
    this.classificationVariants =
        ClassificationVariantSummaryResource.convert(
            version.getPublicClassificationVariants(), language);
  ...
  private boolean filterFuture(CorrespondenceTable correspondenceTable) {
    ...
    // if either source or target From date is later than current date we consider this a future
    // correspondence
