...
Klass is Statistics Norway's system for documentation of classifications and code lists.
This API provides read-only access to classifications and code lists.

Classifications are "official", and in a classification, the categories at each level must be mutually exclusive and complete, i.e. the classification contains all categories that belong to the area covered by the classification.
...
We now provide OpenAPI documentation with a Swagger UI.
Available here: https://data.ssb.no/api/klass/swagger-ui/index.html

=== REST Client

Examples in this documentation use https://en.wikipedia.org/wiki/CURL[curl].
The `-i` parameter in the examples includes the HTTP response headers.
Be aware that when running Curl on MS Windows all single ' have to be replaced by double ".
If Curl is not installed, an alternative is to use any REST client to explore Klass Rest interface, like https://www.getpostman.com/product/api-client[Postman] or address field in the browser.
...
Whenever an error response (status code >= 400) is returned, the response will have a body describing the problem.
For example, a request for a non-existent classification will return the following responses depending on the specified content type.

==== XML

include::{snippets}/error-example-xml/curl-request.adoc[]

will produce a `404 Not Found` response:

include::{snippets}/error-example-xml/http-response.adoc[]

==== JSON

include::{snippets}/error-example-json/curl-request.adoc[]

will produce a `404 Not Found` response:

include::{snippets}/error-example-json/http-response.adoc[]

==== CSV

include::{snippets}/error-example-csv/curl-request.adoc[]

will produce a `404 Not Found` response.
Since CSV has no defined error format, this is a plain string.

include::{snippets}/error-example-csv/http-response.adoc[]

=== Output formats

Default output format is XML.
JSON is optional, and in many cases CSV.
Use the standard Accept header to specify the desired format.

==== Deprecation warning

It was previously possible to get output in the supported output formats without using the Accept header by using the dot-prefix `.json`, `.csv` or `.xml` in the URL before the request parameters.
**This is now deprecated and will be removed presently.
All clients must update to avoid being broken.**.
...
In PxWebApi references to a classification in Klass has the form [.small]#urn:ssb:classification:klass:ID#.
This can be rewritten using the classification ID to [.underline]#httр://data.ssb.no/api/klass/v1/classifications/ID#.
...
include::{snippets}/classifications-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/search-optional-parameters-example/query-parameters.adoc[]

[[classification]]
=== classifications by ID

Get details of a classification.
To get codes from the classification use operations specified in <<classification-links, Links>>
...
include::{snippets}/classification-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/codes-optional-parameters-example/query-parameters.adoc[]

=== codesAt

Used to get codes from a classification.
A date is specified when requesting the codes, and the response will include all codes valid at the specified date.
...
include::{snippets}/codes-at-optional-parameters-example/query-parameters.adoc[]

=== versions by ID

Used to get a classification version by ID.
The ID is shown under classifications.
...
include::{snippets}/versions-request-parameters-example/query-parameters.adoc[]
...
include::{snippets}/variant-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/variant-at-optional-parameters-example/query-parameters.adoc[]

=== variants by ID

Used to get a classification variant by ID.
The ID is shown under versions.
...
include::{snippets}/variants-request-parameters-example/query-parameters.adoc[]
...
A range is specified when requesting the correspondence mappings, and the response will for each correspondence map indicate its valid range (validFrom/validTo).
For more information about the range see <<_range, range>>.
...
include::{snippets}/corresponds-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/corresponds-at-optional-parameters-example/query-parameters.adoc[]
...
The ID is shown under <<_versions_by_id, versions by ID>>.
This method can also be used within versions of the same classification.
...
include::{snippets}/correspondence-tables-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/changes-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/classification-families-optional-parameters-example/query-parameters.adoc[]
...
include::{snippets}/classification-family-optional-parameters-example/query-parameters.adoc[]
...
If now increasing the range to also include 2014, it is seen that some codes are not valid within the whole range.
Some codes end at 2014 and some codes start in 2014.
...
| selectCodes = 3004-34* | range from 3004 to highest code starting with 34
|===
...
When the response contains a collection of resources (e.g. classifications or searchResults), the response will limit the returned collection to a page of configurable size.
