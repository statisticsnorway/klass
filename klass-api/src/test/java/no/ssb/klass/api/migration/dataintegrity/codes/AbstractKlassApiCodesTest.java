...
public abstract class AbstractKlassApiCodesTest extends AbstractKlassApiDataIntegrityTest {
  ...
  static void beforeAllCodes() {
    dateFromToMax =
        generateRandomDate(LocalDate.of(1900, 1, 1), LocalDate.of(2030, 12, 31)).format(formatter);
    dateOne = generateRandomDate(LocalDate.of(1900, 1, 1), LocalDate.of(2030, 12, 31));
    dateTwo = generateRandomDate(LocalDate.of(1900, 1, 1), LocalDate.of(2030, 12, 31));

    dateFromInRangeString =
        dateOne.isBefore(dateTwo) ? dateOne.format(formatter) : dateTwo.format(formatter);
    dateToInRangeString =
        dateOne.isAfter(dateTwo) ? dateOne.format(formatter) : dateTwo.format(formatter);
    ...
    date =
        generateRandomDate(LocalDate.of(1980, 1, 1), LocalDate.of(2020, 12, 31)).format(formatter);
    ...
    paramsPresentationCodePattern.putAll(
        Map.of(FROM, dateFromInRangeString, PRESENTATION_NAME_PATTERN, presentationNamePattern));

    paramsCsvSeparatorAt.putAll(Map.of(DATE, date, CSV_SEPARATOR, ";"));
    paramsCsvFieldsAt.putAll(Map.of(DATE, date, CSV_FIELDS, "name,code"));

    paramsPresentationCodePatternAt.putAll(
        Map.of(DATE, date, PRESENTATION_NAME_PATTERN, presentationNamePattern));
  }
}
