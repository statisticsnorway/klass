...
public class KlassApiClassificationsXmlTest extends AbstractKlassApiClassifications {
  ...
  void getClassificationsXML() {
    Response sourceResponse =
        klassApiMigrationClient.getFromSourceApi(CLASSIFICATIONS_PATH, null, APPLICATION_XML);
    Response targetResponse =
        klassApiMigrationClient.getFromTargetApi(CLASSIFICATIONS_PATH, null, APPLICATION_XML);

    assertApiResponseIsNotNull(sourceResponse);

    assertStatusCodesEqual(
        sourceResponse.getStatusCode(), targetResponse.getStatusCode(), CLASSIFICATIONS_PATH);
    ...
    if (sourceResponse.getStatusCode() != 200) {
      ...
      validatePathListWithObjectsXml(
          sourceResponse,
          targetResponse,
          PAGED_ENTITIES_CONTENTS_CONTENT,
          pathNamesClassificationsPageXml);
  ...
  void getClassificationsIncludeCodeListsXml() {
    Response sourceResponse =
        klassApiMigrationClient.getFromSourceApi(
            CLASSIFICATIONS_PATH, paramsIncludeCodeLists, APPLICATION_XML);
    Response targetResponse =
  ...
  void getClassificationsChangedSinceXml() {
    Response sourceResponse =
        klassApiMigrationClient.getFromSourceApi(
            CLASSIFICATIONS_PATH, paramsChangedSince, APPLICATION_XML);
    Response targetResponse =
        klassApiMigrationClient.getFromTargetApi(
            CLASSIFICATIONS_PATH, paramsChangedSince, APPLICATION_XML);
