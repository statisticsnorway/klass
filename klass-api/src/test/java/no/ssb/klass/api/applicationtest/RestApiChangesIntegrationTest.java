...
//                .prettyPeek()
    ...
    public void restServiceChangesIncludeFutureVersionJSON() {
        given().port(port).accept(ContentType.JSON).param("from", "2013-01-01").param("includeFuture", true)
                .get(REQUEST_WITH_ID_AND_CHANGES, kommuneinndeling.getId())
 //               .prettyPeek()
                .then()
                .statusCode(HttpStatus.OK.value())
                ...
                .body("codeChanges[2].changeOccurred", equalTo(TestDataProvider.TEN_YEARS_LATER_DATE));
    }
...
//                .prettyPeek()
    ...
    public void restServiceChangesIncludeFutureVersionXML() {
        given().port(port).accept(ContentType.XML).param("from", "2013-01-01").param("includeFuture", true)
                .get(REQUEST_WITH_ID_AND_CHANGES, kommuneinndeling.getId())
 //               .prettyPeek()
                .then()
                .statusCode(HttpStatus.OK.value())
                ...
                .body("codeChangeList.codeChangeItem[2].changeOccurred", equalTo(TestDataProvider.TEN_YEARS_LATER_DATE));
    }
...
//                .prettyPeek()
    ...
    @Test
    public void restServiceChangesIncludeFutureVersionCSV() {
        given().port(port).accept(ContentType.XML).param("from", "2013-01-01").param("includeFuture", true)
                .get(REQUEST_WITH_ID_AND_CHANGES, kommuneinndeling.getId())
 //               .prettyPeek()
                .then()
